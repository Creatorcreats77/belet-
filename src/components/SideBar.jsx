...
const SideBar = ({ isOpen, onToggle, focusedIndex, onInfo }) => {
  const menuItems = [
        ...
        <div className="bg-gray-900 h-full p-12 flex flex-col gap-6 w-[464px]">
          ...
          <div className="flex flex-col gap-6 p">
            <div className="flex items-center gap-6">
              <div className={` ${focusedIndex === 0 ? "ring-4 ring-white bg-blue-600 w-16 h-16 rounded-md flex items-center justify-center" : "bg-blue-600 w-16 h-16 rounded-md flex items-center justify-center"}`}>
                <span className="text-lg">P</span>
              </div>
              {isOpen && (
                <div>
                  <p className="text-3xl font-semibold text-white">Profil</p>
                  <p className={`${focusedIndex === 0 ? "text-3xl text-white" : "text-3xl text-gray-400"}`}>99364629645</p>
                </div>
              )}
            </div>
            <div className="flex items-center gap-6">
              <div className={`${focusedIndex === 1? "ring-4 ring-white bg-gradient-to-r from-blue-500 to-orange-500 w-16 h-16 rounded-md flex items-center justify-center" : "bg-gradient-to-r from-blue-500 to-orange-500 w-16 h-16 rounded-md flex items-center justify-center"}`}>
              ...
              {isOpen && (
                <div>
                  ...
                  <p className={`${focusedIndex === 1 ? "text-3xl text-white" : "text-3xl text-gray-400"}`}>0-12</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="mt-auto flex flex-col gap-6 text-gray-400 text-3xl">
            <span key = {(menuItems.length + 2)} className={`${focusedIndex === (menuItems.length + 2) ? "text-3xl text-white" : "text-3xl text-gray-400"}`}>Sazlamalar</span>
            <span key = {(menuItems.length + 2) + 1} className={`${focusedIndex === (menuItems.length + 2) + 1 ? "text-3xl text-white" : "text-3xl text-gray-400"}`}>Goldaw</span>
          </div>
        </div>
